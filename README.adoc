...
----
$>keytool -genkey -alias mycert -keyalg RSA -sigalg MD5withRSA -keystore keystore.jks -storepass password -keypass password -validity 9999
...
Import Certificate:
```
sudo cp cert.pem /etc/pki/ca-trust/source/anchors/mycert.pem
update-ca-trust
```
...
```
openssl pkcs12 -in keystore.p12 -out enc.pem
openssl rsa -in <Encrypted key filename>  -out enc.key
